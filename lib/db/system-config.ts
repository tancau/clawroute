 ...
 * 读取返回 null/[]、写入返回 false，调用方（lib/config）已处理这些默认值。
 */
...
export async function ensureSystemConfigTable() {
  ...
  try {
    ...
    try {
      await db`CREATE INDEX IF NOT EXISTS idx_system_config_type ON system_config(type)`;
    } catch (err) {
      logger.warn({ err }, 'system_config index creation (may already exist)');
    }
  } catch (err) {
    logger.error({ err }, 'ensureSystemConfigTable failed');
  }
}
