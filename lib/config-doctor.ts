...
/**
 * Validate a HopLLM config and produce a health report.
 */
export function diagnoseConfig(config: Record<string, unknown>): HealthReport {
  ...
  if (!models) {
    issues.push({ severity: 'error', category: 'structure', message: 'Missing models section', detail: 'hopllm.json must have a "models" key with providers.', fix: 'Add: "models": { "mode": "merge", "providers": { ... } }' });
    score -= 30;
  }

  if (!agents) {
    issues.push({ severity: 'warning', category: 'structure', message: 'Missing agents section', detail: 'No agents.defaults configured. HopLLM will use default model selection.', fix: 'Add: "agents": { "defaults": { "model": { "primary": "..." } } }' });
    score -= 15;
  }
  ...
  for (const [pid, pdata] of Object.entries(providers)) {
    ...
    if (!pdata.apiKey || (typeof pdata.apiKey === 'string' && pdata.apiKey.trim() === '')) {
      issues.push({ severity: 'warning', category: 'auth', message: `Provider "${pid}" has no API key`, detail: 'HopLLM will fail to authenticate with this provider.', fix: `Add your API key to provider "${pid}"` });
      score -= 8;
    }
  ...
  if (primary) {
    ...
    issues.push({ severity: 'warning', category: 'model-ref', message: 'No primary model set', detail: 'agents.defaults.model.primary is not configured. HopLLM will use its default.', fix: 'Set "primary": "provider/model-id" in agents.defaults.model' });
    score -= 10;
  }
  ...
  if (primary && fallbacks.length === 0) {
    issues.push({ severity: 'info', category: 'best-practice', message: 'No fallback models configured', detail: 'If your primary goes down, HopLLM has no backup. Adding fallbacks improves reliability.', fix: 'Add 1-3 fallback models for resilience.' });
    score -= 3;
  }
