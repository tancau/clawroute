...
import { keyManager } from '../proxy/key-manager';
import { getModelsByIntent as getModelsByIntentFromDB, getModelById as getModelByIdFromDB } from '../../db/model-catalog';

/**
 * 从数据库或静态配置获取候选模型
 * 优先从 model_catalog 数据库查询，fallback 到 providers.ts 静态配置
 */
function getCandidatesForIntent(intent: string): ModelCapability[] {
  // 1. 尝试从数据库查询
  try {
    const dbModels = getModelsByIntentFromDB(intent);
    if (dbModels.length > 0) {
      return dbModels.map(m => ({
        model: m.model_id,
        provider: m.provider,
        intents: m.intents ? JSON.parse(m.intents) : [intent],
        contextWindow: m.context_window || 128000,
        inputCost: m.input_cost_1m,
        outputCost: m.output_cost_1m,
        avgLatency: m.avg_latency_ms ?? undefined,
        qualityScore: m.quality_score ?? undefined,
        features: m.features ? JSON.parse(m.features) : undefined,
      }));
    }
  } catch {
    // DB 不可用，fallback
  ...
  // 2. Fallback: 静态配置
  return getModelsForIntent(intent);
}
...
export const RouteTool: Tool<typeof RouteInputSchema, RouteOutput> = {
  ...
  async call(input: RouteInput, context) {
    ...
    // 1. 获取意图对应的候选模型（优先从 DB，fallback 到静态配置）
    const candidates = getCandidatesForIntent(input.intent);
    
    if (candidates.length === 0) {
      throw new Error(`No models found for intent: ${input.intent}`);
    }

    // 1.5 过滤掉没有 API key 的 provider 的模型
    ...
    // 如果所有模型都没有 key，回退到全部候选（让 proxy 层报错更清晰）
    const effectiveCandidates = availableCandidates.length > 0 ? availableCandidates : candidates;

    // 2. 应用约束过滤
    let filtered = applyConstraints(effectiveCandidates, input.constraints, input.contextLength);
